<%@ page language="java" contentType="text/html; charset=UTF-8"
	pageEncoding="UTF-8"%>

<!DOCTYPE html>
<html>

<body class="">
			<!-- header -->
		<jsp:include page="../framework/header.jsp"></jsp:include>
		<div class="container">

		
			<!-- menu -->
			<jsp:include page="../framework/menu.jsp"></jsp:include>
			

			<div id="frameDiv" class="frame">
				<div class="contentContainer">
					<div class="content">
						

						<div id="mainFrame" class="projectContent">
						<!-- content -->
							   <div style="width:1100px;height:450px;">
							   <div class="informationArea">
							   <form action=''  method="post" onsubmit="" >
							   <div class="box" style="text-align:center;line-height:400px;margin-left:100px;">
								<span class="label" style="font-size:40px;color:blue;">Welcome to the Student Testing System!</span> 
								</div>
							   </form>
							   </div>
							   </div>
						</div>

				<!-- footer -->
			<jsp:include page="../framework/footer.jsp"></jsp:include>			

				</div>
			</div>
		</div>
	</div>




</body>


</html>
